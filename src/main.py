...
from src.elnine_client import ElNineClient, ElNineError
...
def _fetch_from_elnine(config: Config, windows: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    if not config.use_elnine_fallback:
        raise ElNineError("USE_ELNINE_FALLBACK=false: scraper de ELNINE desactivado.")

    logger.info("Usando ELNINE como fallback...")
    client = ElNineClient(cache_dir=config.project_root / ".cache")
    ...
    def fetch_date(target_date: date) -> list[dict[str, Any]]:
        raw_matches = client.get_matches_by_date(target_date)
        return normalize_elnine_matches(raw_matches, config.timezone)

    upcoming_matches: list[dict[str, Any]] = []
    for window in upcoming_windows:
        assert isinstance(window, dict)
        assert isinstance(window["date"], date)
        upcoming_matches.extend(fetch_date(window["date"]))

    return {
        "yesterday_matches": _sort_matches(fetch_date(yesterday_window["date"])),
        "today_matches": _sort_matches(fetch_date(today_window["date"])),
        "upcoming_matches": _sort_matches(upcoming_matches),
    }
...
def build_report_context(config: Config) -> dict[str, Any]:
    ...
    try:
        ...
        if config.use_elnine_fallback:
            source_label = "ELNINE scraper"
            try:
                matches = _fetch_from_elnine(config, windows)
            except ElNineError as elnine_exc:
                fallback_reason = f"{fallback_reason} | ELNINE tampoco respondió: {elnine_exc}"
                logger.warning("ELNINE no respondió: %s", elnine_exc)
