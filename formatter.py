...
def _format_match_section(title: str, matches: list[dict[str, Any]], today_iso: str | None = None) -> str:
    if not matches:
...
def render_daily_telegram(context: dict[str, Any]) -> str:
    ...
    parts.extend(
        [
            "",
            _format_match_section("Ayer", context.get("yesterday_matches", []), today_iso=today_iso),
            "",
            _format_match_section("Hoy", context.get("today_matches", []), today_iso=today_iso),
            "",
            _format_match_section("Próximos partidos", context.get("upcoming_matches", []), today_iso=today_iso),
        ]
    )
